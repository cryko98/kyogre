...
const MemeGenerator: React.FC = () => {
  ...
  return (
    <section id="meme" className="py-20 bg-transparent border-y border-red-900/20">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl md:text-6xl font-black mb-4 uppercase italic tracking-tighter">
            WHALE <span className="text-red-600">BURN</span> FORGE
          </h2>
          <p className="text-slate-500 text-[10px] tracking-[0.4em] uppercase mb-12">
            AI Meme Engine • Powered by $BurnWhale Ashes
          </p>
          
          <div className="bg-slate-950/60 backdrop-blur-md p-6 md:p-10 border border-red-900/30 shadow-2xl mb-10 relative overflow-hidden">
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              <input 
                type="text"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Ex: A massive whale surfing on a wave of burning Solana coins..."
                className="flex-1 bg-black/60 border border-red-900/40 p-4 text-white outline-none focus:border-red-600 font-mono text-sm"
              />
              <div className="flex gap-2">
                <button 
                  onClick={() => startGeneration()}
                  disabled={loading}
                  className="bg-red-600 hover:bg-red-700 disabled:opacity-40 text-white font-black px-8 py-4 uppercase tracking-tighter transition-all whitespace-nowrap shadow-[0_0_20px_rgba(239,68,68,0.4)]"
                >
            ...
            <div className="aspect-square w-full max-w-[500px] mx-auto bg-black/40 border border-red-900/20 flex items-center justify-center relative overflow-hidden group">
              {loading ? (
                <div className="text-center z-10">
                  <div className="w-16 h-16 border-4 border-red-600 border-t-transparent rounded-full animate-spin mx-auto mb-4 shadow-[0_0_20px_rgba(239,68,68,0.3)]"></div>
                ...
                <div className="w-full h-full relative">
                  <img src={result} alt="Generated Whale" className="w-full h-full object-cover animate-in fade-in duration-700" />
                  <a 
                    href={result} 
                    download={`burn-whale-meme-${Date.now()}.png`}
